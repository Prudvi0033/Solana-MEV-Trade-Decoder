export function detectSwapTransaction(tx, DEX_PROGRAM_IDS) {
  ...
  if (tx.transaction?.message?.accountKeys) {
    ...
    return {
      ...
      swapDetected: false,
      swapDetails: null,
      error: 'Unable to parse transaction format'
    };
  }
  ...
  function isRelevantTokenInstruction(ix) {
    ...
    if (DEX_PROGRAM_IDS[pid]) {
      if (ix.parsed?.type && ['swap', 'exchange', 'transfer'].includes(ix.parsed.type.toLowerCase())) {
        return true;
      }
    }
    return false;
  ...
  instructions.forEach((ix) => {
    const pid = accountKeys[ix.programIdIndex];
    if (!IGNORED_PROGRAM_IDS.has(pid)) {  // ✅ FIXED: was using [pid] instead of .has(pid)
  ...
  function detectSwapPattern() {
    ...
    balanceChanges.forEach(change => {
      ...
      if (Math.abs(change.change) > 0.000001) { // Ignore dust
    ...
    for (const [owner, changes] of ownerChanges) {
      if (changes.length >= 2) {
        ...
        if (gains.length >= 1 && losses.length >= 1 && uniqueMints.size >= 2) {
          ...
          const swappedDifferentTokens = [...gainMints].some(mint => !lossMints.has(mint)) &&
                                       [...lossMints].some(mint => !gainMints.has(mint));
          
          if (swappedDifferentTokens) {
            console.log(`✅ SWAP DETECTED! Owner: ${owner.slice(0,8)}...`);
            console.log(`   📈 Gained: ${gains.map(g => `${g.amount} ${g.mint.slice(0,8)}`).join(', ')}`);
            console.log(`   📉 Lost: ${losses.map(l => `${l.amount} ${l.mint.slice(0,8)}`).join(', ')}`);
            return {
              isSwap: true,
              owner,
              tokensIn: losses.map(l => ({mint: l.mint, amount: Math.abs(l.change)})),
